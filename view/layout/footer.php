<footer class="footer">
    <div class="footer__container">
        
        <div class="footer__col">
            <h3 class="footer__title">Masjid Hamzah</h3>
            <p class="footer__text">
                Wadah pembinaan iman, takwa, dan tempat ibadah yang nyaman bagi jamaah Kelnyawit dan sekitarnya. Melayani umat dengan transparansi dan kehangatan.
            </p>
        </div>

        <div class="footer__col">
            <h3 class="footer__title">Tautan Pintas</h3>
            <ul class="footer__links">
                <li><a href="index.php" class="footer__link">Beranda</a></li>
                <li><a href="profil.php" class="footer__link">Profil Masjid</a></li>
                <li><a href="kegiatan.php" class="footer__link">Kegiatan</a></li>
                <li><a href="jadwal-sholat.php" class="footer__link">Jadwal Sholat</a></li>
            </ul>
        </div>

        <div class="footer__col">
            <h3 class="footer__title">Kontak Kami</h3>
            <div class="social-icons">
                <a href="#"><i class="fab fa-facebook-f"></i></a>
                <a href="#"><i class="fab fa-twitter"></i></a>
                <a href="#"><i class="fab fa-instagram"></i></a>
                <a href="#"><i class="fab fa-linkedin-in"></i></a>
            </div>
        </div>

    </div>

    <div class="footer__bottom">
        <p>&copy; <?php echo date('Y'); ?> Masjid Hamzah Kelnyawit. All Rights Reserved.</p>
    </div>
</footer>

<script src="/WEBSITE-MASJID_KELNYAWIT/assets/js/script.js"></script>
</body>
</html>